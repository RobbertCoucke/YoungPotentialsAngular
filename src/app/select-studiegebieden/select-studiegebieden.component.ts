 ...
 * ! selectieveld studiegebieden --> NgSelect module: https://github.com/ng-select/ng-select
 */
...
/**
 * * Interfaces voor studiegebied en opleiding
 *
 */
...
export class SelectStudiegebiedenComponent {
  ...
  @Output() handleStudiegebieden: EventEmitter<
    Studiegebied[]
  > = new EventEmitter<Studiegebied[]>();
  ...
  /**
   * @description verbind geselecteerde studiegebieden met de vacature
   */
  handleSelected() {
    this.tags = [];
          ...
          s.opleiding.forEach(o => {
            if (o.naamOpleiding === element) {
              if (this.tags.filter(e => e.id === s.id).length > 0) {
                ...
                var index = this.tags.indexOf(
                  this.tags.find(e => e.id === s.id)
                );
   ...
   * Eerst wordt een nieuwe array gemaakt met alle opleidingen
   * Dan halen we het studiegebiedID op waartoe de opleidingen behoren
   * De arrays worden samengevoegd tot 1 array: totaal
   */
  opvullenArray() {
