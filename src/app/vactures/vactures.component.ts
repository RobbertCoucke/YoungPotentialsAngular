...
export class VacturesComponent implements OnInit {
  ...
  favorites: Favoriet[] = [];
  favoriteError =
    "liking and unliking offers will not be saved unless you login";
  items: any[] = [];
  ...
  fillVacatures() {
    ...
    if (this.currentUser != null && this.currentUser.role == Role.User) {
      this.favoriteService
        .getAllFavoritesFromUserId(this.currentUser.id)
        .subscribe(f => {
          this.favorites = f;
          f.forEach(element =>
            this.vacatures.push(new Favoriet(element.id, element.vacature))
          );
